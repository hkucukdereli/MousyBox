...
if __name__ == "__main__":
    ...
    fname = "C:\Users\hakan\Dropbox\\for Hakan\Arduino\Cued Reward Scripts\mousybox\data\8621\8621_Day13_NoStim.csv"
    [data, trialNum, Trials_list] = loadData(fname)
    Licks = getLicks(trialNum, Trials_list, th= 50)
    Licks = findLicks(trialNum, Licks)

    bins = np.linspace(5000, 25000, 1000)
    countList = []
    for i in range(1,41):
        ...
        for lick in Licks[i].index:
            if Licks[i]['Stamps'].iloc[lick] == 1:
                ...
                tempDF = pd.DataFrame([Licks[i]['LickTime'].iloc[lick]], columns= [i])
                tempList = tempList.append(tempDF)
        print np.digitize(tempList, bins)
        #print groups.mean()
