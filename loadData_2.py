...
if __name__ == "__main__":
    ...
    nameList = ["\7792\7792_Day19_Reward.csv", "\7793\7793_Day16_Reward.csv", "\7795\7795_Day29_Reward10s.csv", "\8055\8055_Day17_Reward.csv",
                "\8351\8351_Day21_Reward10s.csv", "\8608\8608_Day15_Reward10s.csv", "\8621\8621_Day16_Reward_10s.csv", "\8622\8622_Day12_Reward10s.csv"]
    #fname = "C:\Users\hakan\Dropbox\\for Hakan\Arduino\Cued Reward Scripts\mousybox\data\8621\8621_Day16_Reward_10s.csv"
    fname = "C:\Users\hakan\Dropbox\\for Hakan\Arduino\Cued Reward Scripts\mousybox\data" + nameList[jk]

    for jk in range(0, len(nameList)):
        ...
        for i in range(1,41):
            tempList = []
            for lick in Licks[i].index:
                if Licks[i]['Stamps'].iloc[lick] == 1:
                    #print Licks[i]['LickTime'].iloc[lick], Licks[i]['Stamps'].iloc[lick]
                    tempList.append(Licks[i]['LickTime'].iloc[lick])

            count = 0
            for each in tempList:
                if each > 5000 and each < 10000:
                    count += 1
            countList.append(count)
        mu = np.mean(countList)
        sigma = np.std(countList)
        print mu, sigma

        binT = 1000
        zscoreList = []
        sumList = []
        for k in np.arange(0, 25000, binT):
            countList = []
            for i in range(1,41):
                tempList = []
                for lick in Licks[i].index:
                ...
                count = 0
                for each in tempList:
                    if each > k and each <= k + binT:
                        count += 1
                countList.append(count)
            zscore = (np.mean(countList) - mu) / sigma
            print '(', k, k+binT, ')', zscore, np.mean(countList), np.sum(countList)
            zscoreList.append(zscore)
            sumList.append(np.sum(countList))
        zList = pd.DataFrame(zscoreList)
        zList.to_csv(fname[:-4] + "__zscore.csv")
        sumList = pd.DataFrame(sumList)
        sumList.to_csv(fname[:-4] + "__count.csv")
